...
Question 6 :
1. For each of the four features f1, . . . , f4, drop that feature from both Xtrain and Xtest. 
...
def featureDrop(featureName=None):
    ...
    print("Accuracy from logistic regression model after dropping " +str(featureName) + ": " + str(round(np.mean(predictions == Y_test.ravel())*100,2))) #compare predictions from the KNN model against Y_test output.
